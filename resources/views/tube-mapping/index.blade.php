          ...
          <table class="border-collapse text-[10px] select-none">
            <thead>
              <tr>
                <th class="w-6"></th>
                @for($c=1;$c<=$maxCol;$c++)
                  <th class="text-slate-500 font-normal px-1">{{ $c }}</th>
                @endfor
              </tr>
            </thead>
            <tbody>
              @for($r=1;$r<=$maxRow;$r++)
                <tr>
                  <td class="text-slate-500 pr-1">{{ $r }}</td>
                  @for($c=1;$c<=$maxCol;$c++)
                    ...
                    @if($t)
                      <td class="p-[1px]">
                        <button
                          type="button"
                          @click="selectTube(@js($t->tube_id), @js($t->material), {{ $t->current_thickness_mm }}, {{ $t->min_design_thickness_mm }}, {{ $t->remaining_life_months }}, {{ $t->creep_pct }}, @js($t->status), {{ $t->corrosion_detected ? "true" : "false" }})"
