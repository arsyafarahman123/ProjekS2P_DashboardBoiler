...
<?php $__env->startSection('title', 'S2P PLTU Cilacap — Boiler 3D Digital Twin'); ?>
            ...
            <div style="display:flex;justify-content:flex-start;align-items:center;margin-bottom:18px;flex-wrap:wrap;gap:22px;position:relative;z-index:1;background:rgba(255,255,255,0.03);padding:10px 16px;border-radius:4px;width:100%;">
                ...
                <div style="margin-left:auto;display:flex;align-items:center;gap:14px;">
                    <?php if(auth()->guard()->check()): ?>
                        <span style="color:#9fb0c3;font-size:11.5px;font-weight:600;">👤 <?php echo e(auth()->user()->name); ?></span>
                    ...
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" style="color:#8fb4d6;font-size:11px;font-weight:700;letter-spacing:1px;text-decoration:none;border:1px solid rgba(255,255,255,0.18);border-radius:3px;padding:6px 12px;">ADMIN LOGIN</a>
                    <?php endif; ?>
                </div>
            </div>
...
const DRAWING_UNIT = <?php echo json_encode($drawingUnit, 15, 512) ?>;
// Status login admin (menentukan tombol hapus area di daftar Risk Summary)
const IS_ADMIN = <?php echo json_encode(auth()->check(), 15, 512) ?>;
const AREA_BASE_URL = <?php echo json_encode(url('/admin/areas'), 15, 512) ?>;
...
if (addAreaBtn) {
    ...
    async function submitArea(){
        ...
        const unit = document.getElementById('unit-dd').value;
        try {
            const res = await fetch(<?php echo json_encode(route('areas.store'), 15, 512) ?>, {
                method: 'POST',
                headers: {
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Data D\ProjekS2P_DashboardBoiler\resources\views/dashboard.blade.php ENDPATH**/ ?>